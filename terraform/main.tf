...
resource "aws_cloudwatch_log_group" "lumio_beat" {
  name              = "/ecs/lumio-beat"
  retention_in_days = 7

  tags = merge(local.common_tags, { Name = "/ecs/lumio-beat" })
}
...
# ---------------------------------------------------------------------------
# ECS Task Definition — lumio-beat (Celery Beat scheduler)
# ---------------------------------------------------------------------------
resource "aws_ecs_task_definition" "beat" {
  ...
  container_definitions = jsonencode([
    {
      ...
      logConfiguration = {
        logDriver = "awslogs"
        options = {
          "awslogs-group"         = aws_cloudwatch_log_group.lumio_beat.name
          "awslogs-region"        = var.aws_region
  ...
  tags = merge(local.common_tags, { Name = "lumio-beat" })
}
...
# CRITICAL: desired_count = 1 — only one Beat process must ever run at a time
# to avoid duplicate scheduled task firings.
resource "aws_ecs_service" "beat" {
  ...
  tags = merge(local.common_tags, { Name = "${local.name_prefix}-beat" })
}
